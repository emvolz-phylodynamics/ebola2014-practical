# Credit: Simon frost, https://github.com/sdwfrost/exploratory-phylodynamics
require(ape)
require(INLA)
...
calculate.heterochronous.skyride <- function(tree){
  ...
  nrows <- min( c( length(event), length(time_aware), length(E)) ) 
  data_mod<-data.frame(y=event[2:nrows],event=event[2:nrows],time=time_aware[2:nrows],E=log(E)[1:(nrows-1)])
  ...
  nrows = min( c( length(grid.points.scaled1), length(sr.median), length(sr.lc), length(sr.uc)) )
  data.frame(time=grid.points.scaled1[1:nrows],sr.median=sr.median[1:nrows],sr.lc=sr.lc[1:nrows],sr.uc=sr.uc[1:nrows])
}
